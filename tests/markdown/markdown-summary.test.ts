  ...
  it("keeps fenced code content unchanged", () => {
    expect(
      normalizeSummaryMarkdownHeadings(
        ["```sh", "# still a shell comment", "## still a shell comment", "```"].join("\n"),
      ),
    ).toBe(["```sh", "# still a shell comment", "## still a shell comment", "```"].join("\n"))
  })

  it("normalizes only the MarkdownSummary input", () => {
    const renderer = fs.readFileSync("apps/web/components/content/markdown-renderer.tsx", "utf8")

    expect(renderer).toContain('import { normalizeSummaryMarkdownHeadings } from "@/lib/markdown-summary"')
    expect(renderer).toContain("content={normalizeSummaryMarkdownHeadings(content)}")

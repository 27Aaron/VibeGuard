...
describe("admin feeds layout", () => {
  it("presents the create-feed enabled option as a balanced setting row", () => {
    const form = fs.readFileSync(
      "apps/web/components/admin/create-feed-form.tsx",
      "utf8",
    );
    ...
    expect(form).toContain(
      "h-4 w-4 shrink-0 rounded border-black/20 accent-emerald-800",
    );
  ...
  it("gives configured sources table a padded surface instead of hugging the card edge", () => {
    const page = fs.readFileSync(
      "apps/web/app/[lang]/admin/feeds/page.tsx",
      "utf8",
    );
    ...
    expect(layoutTokens).toContain(
      "overflow-hidden rounded-[1.25rem] border border-black/5",
    );

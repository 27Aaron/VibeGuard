...
describe("public homepage icon controls", () => {
  it("uses icon-only actions for search, clear, and pagination controls", () => {
    ...
    expect(page).toContain('buttonVariants({ size: "icon", variant: "outline" })')
    expect(page).toContain('buttonVariants({ size: "icon-sm", variant: "outline" })')
    ...
    expect(page).toContain('<ChevronLeft className="size-4" />')
    expect(page).toContain('<ChevronRight className="size-4" />')
  })
})

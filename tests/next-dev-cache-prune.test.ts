...
import {
  pruneNextDevCache,
} from "../scripts/prune-next-dev-cache.mjs"
...
function createTempAppDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vibeguard-next-dev-cache-"))
  tempDirs.push(dir)
  return dir
}
...
describe("pruneNextDevCache", () => {
  it("is wired into the web dev script before Next.js starts", () => {
    ...
    expect(webPackage).toContain('node ../../scripts/with-env.mjs --local-defaults next dev')
  })

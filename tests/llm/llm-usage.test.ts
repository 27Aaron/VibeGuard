  ...
  it("should return relevance result and usage from the API", async () => {
    const create = vi.fn().mockResolvedValue({
      choices: [
        {
          message: {
            content: JSON.stringify({ relevant: true, reason: "security content" }),
          },
          finish_reason: "stop",
        },
      ],
    ...
    expect(result.result).toEqual({ relevant: true, reason: "security content" })
    expect(result.usage).toEqual({
      promptTokens: 300,
      completionTokens: 20,
      totalTokens: 320,

...
describe("security API documentation", () => {
  ...
  const openapiRoute = fs.readFileSync("apps/web/app/[lang]/openapi/route.ts", "utf8")
  ...
  it("ships localized OpenAPI documents and keeps the default Chinese-compatible alias", () => {
    ...
    expect(openapiEn).toContain("Batch check package coordinates against the local security mirror")

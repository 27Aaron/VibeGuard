...
describe("W15: deleteCachedOsvFile uses unlink, not recursive rm", () => {
  ...
  it("throws when trying to delete a non-existent file instead of silently succeeding", async () => {
    await expect(deleteCachedOsvFile("/tmp/vibeguard-w15-nope.json")).rejects.toThrow()
  })
...
describe("W16: assertSafeFileName rejects NULL bytes", () => {
  ...
  it("rejects filenames containing null bytes", () => {
    expect(() =>
      buildOsvCachePath({ repoRoot: "/tmp", ecosystem: "npm", fileName: "bad\0.json" })
    ).toThrow("Unsafe OSV cache filename")
  })

  it("rejects filenames with path traversal", () => {
    expect(() =>
      buildOsvCachePath({ repoRoot: "/tmp", ecosystem: "npm", fileName: "../etc/passwd" })
    ).toThrow("Unsafe OSV cache filename")
  })
  ...
  it("accepts valid filenames", () => {
    expect(() =>
      buildOsvCachePath({ repoRoot: "/tmp", ecosystem: "npm", fileName: "MAL-2026-001.json" })
    ).not.toThrow()
  })
})
...
describe("W41: sync marks partial success correctly", () => {
  it("partial success is not marked as FAILED", async () => {
    ...
    const { parseModifiedIdCsv } = await import("../../packages/content/src/osv/sync")
...
describe("W52: stripHtmlTags removes HTML before classification", () => {
  ...
  it("strips complex HTML with attributes", () => {
    expect(
      stripHtmlTags('<a href="https://example.com">Click here</a>')
    ).toBe("Click here")
  })

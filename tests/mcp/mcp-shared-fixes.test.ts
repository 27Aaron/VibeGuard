...
describe("I33: ToolDefinition has type parameter for inputSchema", () => {
  ...
  it("ToolDefinition interface has a generic type parameter", () => {
    expect(source).toMatch(
      /interface\s+ToolDefinition\s*<\s*T\s+extends\s+Record<string,\s*z\.ZodTypeAny/,
    );
  });

  it("ToolHandler type has a matching generic parameter", () => {
    expect(source).toMatch(
      /type\s+ToolHandler\s*<\s*T\s+extends\s+Record<string,\s*z\.ZodTypeAny/,
    );
  });
});
...
describe("I34: ecosystems/riskCategories come from shared package", () => {
  ...
  it("tools.ts imports MCP_ECOSYSTEMS, MCP_RISK_CATEGORIES from @vibeguard/shared", () => {
    expect(source).toMatch(
      /import\s*\{[^}]*MCP_ECOSYSTEMS[^}]*\}\s*from\s*["']@vibeguard\/shared["']/,
    );
    expect(source).toMatch(
      /import\s*\{[^}]*MCP_RISK_CATEGORIES[^}]*\}\s*from\s*["']@vibeguard\/shared["']/,
    );
  });
  ...
  it("MCP_CHECK_ECOSYSTEMS has the expected values", () => {
    expect([...MCP_CHECK_ECOSYSTEMS]).toEqual([
      "npm",
      "pypi",
      "go",
      "crates-io",
...
describe("I15: SecurityPackageMatchConfidence 'undetermined' semantics", () => {
  ...
  it("confidence values are correct", () => {
    expect([...SECURITY_PACKAGE_MATCH_CONFIDENCE_VALUES]).toEqual([
      "high",
      "medium",
      "low",
      "undetermined",

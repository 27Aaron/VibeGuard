...
describe("MCP documentation page", () => {
  const pageSource = fs.readFileSync("apps/web/app/[lang]/mcp/page.tsx", "utf8")
  ...
  it("adds a copy button next to the MCP endpoint URL", () => {
    expect(pageSource).toContain('import { CopyButton } from "@/components/ui/copy-button"')
    ...
    expect(pageSource).toContain('copiedLabel={lang === "zh" ? "已复制" : "Copied"}')
  })
  ...
  it("keeps available tools readable in a compact two-zone layout", () => {
    ...
    expect(pageSource).not.toContain("flex items-start gap-3 rounded-[0.85rem]")

...
export function buildPackageCheckMeta({
  ...
  const stale =
    !lastSyncedAt || now.getTime() - lastSyncedAt.getTime() > staleAfterMs;
...
function formatPackageCoordinate(name: string, version?: string | null) {
...
function advisoryCveEnrichmentIds(advisory: {
  ...
  return [...advisory.aliases, ...(advisory.upstreamIds ?? [])];
}
...
function formatCveEnrichment(
  row: typeof securityCveEnrichments.$inferSelect,
): SecurityCveEnrichmentResult {
  return {
    cveId: row.cveId,
    title: row.title,
...
export async function checkPackagesAgainstLocalDb(
  ...
  return {
    ...
    findings: findings.sort(
      (left, right) =>
        findingLatestTimestamp(right) - findingLatestTimestamp(left),
    ),
  };
}

...
export const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    description: "查询结构化漏洞公告。支持按包、CVE、风险类型、KEV、CVSS、EPSS 筛选。",
    inputSchema: {
      ...
      riskType: z.enum(["unknown", "vulnerability", "malicious-package"]).optional().describe("风险类型"),
      kev: z.boolean().optional().describe("是否筛选 CISA KEV 记录"),
      cvssMin: z.number().min(0).max(10).optional().describe("最低 CVSS 分数"),
      epssMin: z.number().min(0).max(1).optional().describe("最低 EPSS percentile"),
      limit: z.number().min(1).max(100).optional().describe("返回数量，默认 10"),
    },
    async handler(client, args) {
      ...
      const lines = result.items.map((advisory, index: number) => {
        ...
        const packages = advisory.packageImpacts?.slice(0, 3)
  ...
  },
  {
    ...
    description: "获取单个包的风险画像，包含命中数量、确认影响数量、最高风险和推荐修复版本。",
    inputSchema: {
    ...
    async handler(client, args) {
      ...
      const fixed = summary.recommendedFixedVersions.length > 0
  ...
  },
  {
    ...
    async handler(client, args) {
      ...
      return [
        `CVE: ${result.cveId}`,
        enrichment?.bestCvssScore ? `CVSS: ${enrichment.bestCvssSeverity ?? "UNKNOWN"} ${enrichment.bestCvssScore}` : "",
        enrichment?.epssPercentile ? `EPSS percentile: ${enrichment.epssPercentile}` : "",
        enrichment?.kevListed ? "CISA KEV: yes" : "CISA KEV: no",
        enrichment?.cweIds?.length ? `CWE: ${enrichment.cweIds.join(", ")}` : "",
        enrichment?.kevRequiredAction ? `KEV 要求: ${enrichment.kevRequiredAction}` : "",

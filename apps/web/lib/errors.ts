...
export function normalizeUserFacingError(error: unknown, lang: AppLang = "zh") {
  const fallback = lang === "zh" ? "操作失败，请稍后重试。" : "The operation failed. Please try again."
  ...
  if (message.includes("Request to") && message.includes("status 429")) {
    return lang === "zh" ? "模型服务当前限流，请稍后重试。" : "The model service is rate limited. Please try again later."
  }

  if (message.includes("Request to") && message.includes("status 500")) {
    return lang === "zh" ? "模型服务暂时异常，请稍后重试。" : "The model service is temporarily unavailable. Please try again later."
  }

  if (message.includes("404 Not Found") && message.toLowerCase().includes("openresty")) {
    return lang === "zh"

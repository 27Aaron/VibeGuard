...
export type ArticleRegenerationTarget =
  (typeof ARTICLE_REGENERATION_TARGETS)[number];
...
function hasCompleteContent(
  article: RegeneratableArticle,
  patch: Partial<RegeneratableArticle>,
) {
  const next = {
    ...article,
    ...patch,
...
export function getRegenerationRequirementError(
  ...
  if (
    ...
    FILTERED_BLOCKED_TARGETS.has(target)
  ) {
    return lang === "zh"
      ? "当前文章已被过滤"
...
export async function regenerateArticleTarget(
  ...
  if (input.target === "fetch-source") {
    ...
    const extracted = await dependencies.extractMarkdownFromHtml(
      html,
      input.article.url,
    );
    ...
    const summaryEnResult = await dependencies.summarizeText({
      ...
      systemPrompt: buildLocalizedSummaryPrompt(
        input.settings.summaryPromptEn,
        "en",
      ),
      sourceText: contentMdEn,
    });

    ...
    <div className="rounded-[1.4rem] border border-black/5 bg-white/50 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)] dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-mono text-sm font-semibold text-zinc-950 dark:text-stone-100">
            {path}
          </span>
        </div>
            ...
            return (
              <div className="rounded-xl border border-emerald-200/60 bg-emerald-50/50 p-3 dark:border-emerald-800/40 dark:bg-emerald-950/20">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex items-center gap-1.5">
                      <span className="font-mono text-xs font-semibold text-emerald-900 dark:text-emerald-200">
                        {p.key}
                      </span>
                      <span className="text-[0.65rem] text-emerald-700 dark:text-emerald-400">
                        {p.label}
                      </span>
                    </div>
                  ...
                  <button
                    ...
                    className="cursor-pointer shrink-0 rounded-md p-0.5 text-emerald-600 transition-colors hover:text-emerald-800 dark:text-emerald-400 dark:hover:text-emerald-200"
                ...
                {p.examples && p.examples.length > 0 && (
                  <div className="mt-2 flex flex-col gap-1 border-t border-emerald-200/40 pt-2 dark:border-emerald-800/30">
                    <span className="mb-0.5 text-[0.6rem] font-medium uppercase tracking-wider text-emerald-700/60 dark:text-emerald-400/50">
                      {lang === "zh" ? "示例" : "Examples"}
                    </span>
                    {p.examples.map((ex) => (
                      <a
                        key={ex.label}
                        href={ex.href}
                        className="truncate font-mono text-[0.7rem] text-emerald-800 transition-colors hover:text-emerald-600 dark:text-emerald-300 dark:hover:text-emerald-200"
                      >
                        {ex.label}
                      </a>
                    ))}
